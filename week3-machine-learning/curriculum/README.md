...
9. 각 문제는 **직접 구현할 개념**, **사용 권장 API**, **검산 전용 API**를
   구분한다. 학습 대상 개념을 완성 함수 하나로 숨기지 않고, 기본 배열
   연산은 재구현하지 않는다.

### 교재 정렬과 선수지식 gate

세션은 교재 연결을 다음 세 상태 중 하나로 명시한다.

- `verified`: 원문, 공식 발췌, 저자 자료에서 실제 설명 범위를 확인했다.
- `inferred`: 공개 목차·공식 메타데이터·표준적인 학부 교재 구성을 바탕으로
  주제 연결만 추정했다.
- `unknown`: 주제나 설명 수준을 판단할 근거가 없다.

`inferred`나 `unknown` 상태에서 특정 교재의 정확한 설명 순서·유도 범위·연습문제
수준을 재현했다고 주장하지 않는다. 이때는 `MLAPP-style probabilistic depth`처럼
범위를 제한해 표현하고, 추정한 내용을 Core 회상·채점 과제로 만들지 않는다.

Core는 세션에 명시된 목표, 문제 안에 제공한 정의, 표준 학부 선수지식, 학습자가
실제로 배웠다고 확인한 내용으로만 구성한다. 세션 시작 부분에는 현재 계산에 필요한
기초 정의와 기호를 짧게 다시 제공한다. 진행 중 새로운 선수지식이 발견되면 오답으로
채점하지 않고 guided teaching으로 전환하며, 해당 내용은 설명형 선택 확장으로
분리한다.

API 경계는 다음 기준으로 정한다.

- 현재 배우는 식·축·데이터 변환 전체를 한 호출로 대체하면 검산 전용이다.
- indexing, reduction, shape 조작처럼 식을 구성하는 primitive는 사용한다.
- API 선택 자체가 수치 안정성의 학습 목표이면 필수 구현 도구로 사용한다.
- 이전 세션에서 직접 구현해 이해한 개념은 후속 세션에서 API로 사용할 수 있다.
- 검산 API가 여러 개면 가장 표준적인 선택 하나를 **표준 검산 선택**으로
  표시하고, shape 계약·축 인자·의존성·수치 안정성 관점에서 선택 이유를 적는다.
- 표준 API 코드는 `standard_api.py`에 완성된 읽기 자료로 제공한다. T4는
  이 파일을 참조하는 선택 항목이며, 학습자에게 별도 검산 코드를 요구하지 않는다.
...
8. 문제에 명시되지 않은 새 선수지식이 필요하면 풀이를 중단하고 필요한 정의부터
   가르친다. 배우지 않은 내용을 회상 문제로 취급하거나 채점하지 않는다.
